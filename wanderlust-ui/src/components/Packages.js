...
class Packages extends Component {
  ...
  getPackages = ( continent ) => {
    ...
    var continent1 = continent.substring( 0,1 ).toUpperCase()+continent.substring( 1 ).toLowerCase();
    axios.get( backendUrlPackage + '/destination/' + continent1 )
        .then( ( response ) => {
...
loadBookingPage = ( dealId ) => {
    ...
    this.setState( { show: true, bookingPage: true, showItinerary: false ,dialogBox: true} )
}
            ...
            for( let mypackage of packages ) {
                let element = (
                    <div key={mypackage.destinationId}><br />
                        <div className="card  text-dark package-card" style={{backgroundColor: '#BCC6CC'}} >
                            <div className="card-body row">
                                <div className="col-md-4">
                                    <img className="package-image img-responsive" src={mypackage.imageUrl} alt="destination comes here" />
                                </div>
...
aria-describedby='alert-dialog-description'>
 ...
 <b className='h4 text-primary'style={{fontFamily: "Courier New"}}>Please Login to continue</b>
   </DialogContentText>
 </DialogContent>
   ...
   <Link to='/login'color='primary'  variant="outlined"  className='btn btn-info'> LOGIN</Link>&nbsp;
    ...
    return(
        <div>
            ...
            <Sidebar visible={this.state.showItinerary} position="left" className="p-sidebar-lg" onHide={( e ) => this.setState( { showItinerary: false } )}>
                ...
                <TabView activeIndex={Number( this.state.index )} onTabChange={( e ) => this.setState( { index: e.index } )}>
                    ...
                    <TabPanel header="Book">
                        <h4 className="itenaryAbout text-success text-left">**Charges per person: $ {this.state.deal.chargesPerPerson}</h4>
